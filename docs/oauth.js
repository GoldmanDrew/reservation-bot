/** GitHub OAuth helpers for static Pages (device flow + PKCE via CORS proxy). */
(function (global) {
  ...
  async function signIn(clientId, cfg) {
    ...
    if (!proxyBase(cfg)) {
      throw new Error(
        "OAUTH_PROXY_URL not configured. Deploy docs/oauth-proxy to Cloudflare and set the repo variable. See docs/oauth-proxy/README.md"
      );
    }
  ...
  async function handleCallback(clientId, cfg) {
    ...
    if (!verifier) {
      throw new Error("PKCE verifier missing — use Sign in with GitHub on the dashboard");
    }
  ...
  async function refreshAuthUi(btn) {
    ...
    if (getToken() && u2) {
      ...
      btn.title = "Authorize snipe workflows via GitHub OAuth";
      btn.dataset.signedIn = "0";
    }
  }

  global.ReservationOAuth = {
    loadConfig,
    getToken,
    getUser,
    clearAuth,
